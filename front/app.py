...
if img_file is not None:
    ...
    if st.button('Upload image'):
        ...
        confidence = float(response.headers["confidence"]) - 10
        st.markdown("<h3 style='text-align: center; color: black;'>ECG result: "+response.headers["prediction"]+"</h3>", unsafe_allow_html=True)
        st.markdown("<h5 style='text-align: center; color: black;font-size:10'>Confidence level: "+str(confidence)+"</h5>", unsafe_allow_html=True)
